    ...
    ['$scope',
        ...
        function ($scope, alarmFiredId, $uibModalInstance, alarmFiredService, toastr, $rootScope, onlyRead, responseDispatcherService, geolocationService, $q) {
            ...
            $scope.initPinMap = function () {
                ...
                //    if ($scope.alarmFiredModel.isFall || $scope.alarmFiredModel.isFallNoResponse) {
                //        return $scope.alarmFiredModel.latlng;
                //    }
                //    //Altrimenti ritorno la sua posizione di residenza
                //    else
                //        return $scope.alarmFiredModel.processInstance.patient.latlng;
                //} 

                //var checkPinMap = true;
                //while (checkPinMap) {
                //    if ($scope.alarmFiredModel) {
                //        initPinMapSub();
                //        checkPinMap = false;
                //    }     
                //}

                var latLng = $scope.alarmFiredModel.processInstance.patient.latlng;

                if ($scope.alarmFiredModel.processInstance.patient.latlngLast)
                    latLng = $scope.alarmFiredModel.processInstance.patient.latlngLast;
