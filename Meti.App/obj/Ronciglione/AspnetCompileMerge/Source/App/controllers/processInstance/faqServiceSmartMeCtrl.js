app.controller('faqServiceSmartMeCtrl',
    ['$scope',
        ...
        'httpService',
        'healthRiskService',
        function ($scope, $log, responseDispatcherService, $q, toastr, $rootScope, parameterService, deviceErrorLogService, alarmFiredService, processInstanceService, registryService, base64Service, httpService, healthRiskService) {
            //#region Model
            ...
            $scope.fetchHealthRiskByProcessInstancesRequest = angular.toJson({ 'processInstanceIds': ['f22c7811-e55c-4412-9d2e-aa0700c555ce'] });
            $scope.fetchHealthRiskByProcessInstancesResponse = null;
            ...
            $scope.fetchHealthRiskByProcessInstances = function () {

                var request = angular.fromJson($scope.fetchHealthRiskByProcessInstancesRequest);

                var promise = healthRiskService.fetchByProcessInstanceIds(request);

                promise.then(function (response) {
                    var data = response.data;
                    $scope.fetchHealthRiskByProcessInstancesResponse = angular.toJson(data);
                    toastr.success("Richiesta Eseguita");
                }, function (error) {
                    var data = error.data.data;
                    var vResults = data.validationResults;
