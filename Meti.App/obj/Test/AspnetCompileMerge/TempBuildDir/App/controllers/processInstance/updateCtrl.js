    ...
    ['$scope',
        ...
        function ($scope, $log, responseDispatcherService, processInstanceService, $q, toastr, utilsService, $rootScope, registryService, $state, $window, $uibModal, processService, $stateParams) {
            ...
            $scope.close = function () {
                ...
                var promise = processInstanceService.close(request);

                promise.then(function (response) {
                    var data = response.data;
                    //$scope.processInstanceModel.id = data.id;
                    toastr.success("Il processo è stato chiuso. Non sarà più possibile modificarlo fino ad una prossima apertura. Tutti i dispositivi ad esso associati sono stati liberati.");
                    $window.history.back();
                }, function (error) {
            ...
            $scope.open = function () {
                ...
                var promise = processInstanceService.open(request);

                promise.then(function (response) {
                    var data = response.data;
                    //$scope.processInstanceModel.id = data.id;
                    toastr.success("Il processo è stato aperto. Adesso sarà possibile apportare modifiche. In questo momento non ci sono dispositivi associati.");
                    $window.history.back();
                }, function (error) {
                    responseDispatcherService.printErrorMessages(error.data.data, toastr);
                    $log.error("Errore in processInstance-updateCtrl.update: " + angular.fromJson(error));
